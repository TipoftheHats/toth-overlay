$(document).ready(function (){
    ...
    function showLinks() {	
		//play sound
		setTimeout(function(){$.ionSound.play("socialmedia_in-v2");}, 0);
		...
		function showHat() {
			...
			setTimeout(function(){$('#hatty').css('transform','translate3d( 0, 0px, 0px ) rotateX( 0deg )');}, 500);
			setTimeout(function(){$('#hatty').css('opacity','100');}, 500);
			setTimeout(function(){$('#hatty').css('transform-origin','50% 50%');}, 1000);
			setTimeout(function(){$('#hatty').addClass('animated pulseSmaller');}, 1000);		
		}
    ...
    function hideLinks() {
		//play sound
		setTimeout(function(){$.ionSound.play("socialmedia_out-v2");}, 0);
    ...
    function showAlert(data) {
        if(data.content) {
			//play sound
			setTimeout(function(){$.ionSound.play("alert_in-v2");}, 0);
			
            $('#alerttext').text(''+data.content+'');
			$('#alert').css('width','18.5em');
			$('#alerttitlecontainer').css('background-color','#e7501e');
			setTimeout(function(){$('#exclamation').css('opacity','100');}, 100);
			setTimeout(function(){$('#alerttextcontainer').css('height','3.3em');}, 400);
	...
	function hideAlert() {
		...
		setTimeout(function(){$('#alert').css('width','0');}, 400);
		setTimeout(function(){$('#exclamation').css('opacity','0');}, 1000);  
        setTimeout(function(){$('#alerttitlecontainer').css('background-color','#ffcc28');}, 2000);
    }
	...
	function showLowerThird(data) {	
		...
		if(msgParsed.text && msgParsed.title) {
			...
			$('#lowerthirdtitlecontainer').css('background-color','#e7501e')
                                          .transition(
                                           {'left': '0%'}, 600, 'cubic-bezier(0.260, 0.860, 0.440, 0.985)');
	...
	function hideLowerThird() {
		...
		$('#lowerthirdtitlecontainer').transition(
                                       {'left': '-100%'}, 300, 'cubic-bezier(0.260, 0.860, 0.440, 0.985)');
        ...
        $('#shape').delay(200)
                   .transition(
                    {'left': '-190px'}, 700, 'cubic-bezier(0.260, 0.860, 0.440, 0.985)');
    }
	...
	function playTransition() {	
		//play sound
		setTimeout(function(){$.ionSound.play("transition_in");}, 50);
	...
	function openDoor() {
		//play sound
		setTimeout(function(){$.ionSound.play("transition_out");}, 50);
    ...
    function updateRosters(data) {
        ...
        //show player names
        var redRoster = msgParsed.redRoster; var bluRoster = msgParsed.bluRoster;
        var rosterSize = Object.keys(redRoster).length;
        var leftroster = '', classicon = '', rightroster = '';
        ...
        $('#rosterheader').html('<div class="leftheader cell">Team 1</div><div class="classicon cell"></div><div class="rightheader cell">Team 2</div>');
        ...
        if (rosterSize == 6) { //6v6
            ...
            for (var i = 1; i <= rosterSize+1; i++) {
                $('#classicon > div:nth-child('+i+')').css('background-position-y',-36*(i));
            }  
        }
    }
});
